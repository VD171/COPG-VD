...
static T_Callback orig_callback = nullptr;
static void (*orig_system_property_read_callback)(void*, T_Callback, void*) = nullptr;
...
public:
    void onLoad(zygisk::Api* api, JNIEnv* env) override {
        ...
        if (!versionClass) {
            jclass localVersionClass = env->FindClass("android/os/Build$VERSION");
            if (localVersionClass) {
                versionClass = (jclass)env->NewGlobalRef(localVersionClass);
            ...
            env->DeleteLocalRef(localVersionClass);
        }
    ...
    void postAppSpecialize(const zygisk::AppSpecializeArgs* args) override {
        if (!args || !args->nice_name || package_map.empty() || !buildClass) {
            api->setOption(zygisk::Option::DLCLOSE_MODULE_LIBRARY);
            return;
        }
        const char* package_name = env->GetStringUTFChars(args->nice_name, nullptr);
...
private:
    ...
    void hookNativeGetprop() {
        ...
        if (sym) {
            DobbyHook(sym, (void*)hooked_prop_get, (void**)&orig_prop_get);
        }
    }
    ...
    void hookNativeRead() {
        void* sym = DobbySymbolResolver("libc.so", "read");
        if (sym) {
            DobbyHook(sym, (void*)hooked_read, (void**)&orig_read);
        }
    }
    ...
    void hookJniSetStaticObjectField() {
        void* sym = DobbySymbolResolver("libandroid_runtime.so", "JNI_SetStaticObjectField");
        if (sym) {
            DobbyHook(sym, (void*)hooked_set_static_object_field, (void**)&orig_set_static_object_field);
        }
    }

    static void modify_callback(void* cookie, const char* name, const char* value, uint32_t serial) {
        if (!cookie || !name || !value || !orig_callback) {
            orig_callback(cookie, name, value, serial);
            return;
        }

        std::string prop(name);
        if (prop == "ro.product.brand" && !current_info.brand.empty()) {
            value = current_info.brand.c_str();
        } else if (prop == "ro.product.device" && !current_info.device.empty()) {
            value = current_info.device.c_str();
        } else if (prop == "ro.product.manufacturer" && !current_info.manufacturer.empty()) {
            value = current_info.manufacturer.c_str();
        } else if (prop == "ro.product.model" && !current_info.model.empty()) {
            value = current_info.model.c_str();
        } else if (prop == "ro.build.fingerprint" && !current_info.fingerprint.empty()) {
            value = current_info.fingerprint.c_str();
    ...
    static void my_system_property_read_callback(void* pi, T_Callback callback, void* cookie) {
        if (pi && callback && cookie) orig_callback = callback;
        orig_system_property_read_callback(pi, modify_callback, cookie);
    }

    void hookProperties() {
        void* sym = DobbySymbolResolver(nullptr, "__system_property_read_callback");
        if (sym) {
            DobbyHook(sym, (void*)my_system_property_read_callback, (void**)&orig_system_property_read_callback);
        }
    }
};
